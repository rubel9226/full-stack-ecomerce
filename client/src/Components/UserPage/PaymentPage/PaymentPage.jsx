...
const PaymentPage = () => {
  ...
  // useEffect(() => {
  //   const frontendId = location.state?.orderId || invoiceNo;
  ...
  //       const {data} =await api.post("/orders/payment-sslcommerz", {orderId: orderId});
  //       if(data.success){
  //         console.log(data?.payload);
  //         window.location.href = data?.payload;
  //       }
  //     }else if(paymentMethod === 'cash_on_delivery'){
  //       const {data} =await api.put("/orders/payment-cash_on_delivery", {orderId: orderId});
  //       console.log(data?.payload);
  ...
  // }
    ...
    return (
        <div>
            <div className="space-y-5 bg-[#f5f5f5] min-h-screen p-4">
              {
          ...
          <div className='fixed bg-white left-0 bottom-0 shadow-[0_-4px_5px_rgba(0,0,0,0.1)] shadow-black/20 z-10 w-full'>
              ...
              <div className='flex py-3 w-11/12 mx-auto justify-between items-center'>
                  <div className='flex items-center gap-1 font-semibold'>
                      ...
                      <span className='font-bold text-[15px] text-black'><span className='font-serif'>৳ </span>{pricing?.total}</span>
                  </div>
