...
export default function ShaveTrim() {
  const [products, setProducts] = useState([]);
    ...
    const handleGetPopularSection =async () => {
      setLoading(true);
        try {
            const res = await api.get('/products/home-sections/all-product?section=shavingTrimming');
              ...
              <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5'>
                  {
                      products.map((product, index) => {
                          ...
                          const handleAddHomeSection = async () => {
                              ...
                              try { 
                                  await api.put(`/products/home-sections/delete/${product?.slug}?section=shavingTrimming`); 
                                  toast.success(<span className="capitalize">{`${product.name} delete to Shaving & Trimming section successfully.`}</span>)
                                  handleGetPopularSection();
                              } catch (error) {
                                      ...
                                      <div className='mt-4 flex justify-end'>
                                          ...
                                          <div className="flex justify-center items-center">
                                              <button onClick={handleAddHomeSection} className="font-semibold text-[12px] btn btn-sm btn-error flex transform transition-all hover:scale-105 duration-200 hover:cursor-pointer">
                                                  ...
                                                  {loadingId === product._id ? 'loading...' : 'Delete Section'}
                                              </button>
                                          </div>
