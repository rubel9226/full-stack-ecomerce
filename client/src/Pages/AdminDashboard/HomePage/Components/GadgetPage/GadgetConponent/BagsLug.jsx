          ...
          <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5'>
              {
                  products.map((product, index) => {
                      ...
                      const handleAddHomeSection = async () => {
                          ...
                          try { 
                              await api.put(`/products/home-sections/delete/${product?.slug}?section=bagsLuggage`); 
                              toast.success(<span className="capitalize">{`${product.name} delete to Bugs & Luggage section successfully.`}</span>)
                              handleGetPopularSection();
                          } catch (error) {
