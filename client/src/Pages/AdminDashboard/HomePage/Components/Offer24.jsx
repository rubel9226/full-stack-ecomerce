...
export default function Offer24() {
  const [products, setProducts] = useState([]);
    ...
    const handleGetPopularSection =async () => {
        try {
            const res = await api.get('/products/home-sections/all-product?section=dailyOffer');
  ...
  return (
    <section className="bg-[url('https://res.cloudinary.com/dext9i4ab/image/upload/v1780523844/offer-bg_fhhz3p.webp')] bg-cover bg-center w-full mt-5 md:rounded-2xl">
      ...
      <div className="px-4 py-5 sm:px-5 md:px-7 lg:px-10 xl:px-14 lg:py-8">
        ...
        {/* Timer */}
        <div className="text-white font-bold uppercase flex items-center gap-2 sm:gap-3">

          <h3 className='text-[10px] sm:text-[12px] font-extrabold'>
            ends in
          </h3>

          <div className="text-white flex items-center gap-1 sm:gap-2">

            <span className="bg-white text-[#5261AD] px-1.5 py-1 sm:px-2 sm:py-1.5 rounded-md text-xs sm:text-sm">
              12
            </span>
        ...
        {/* Header */}
        <div className='text-white font-bold flex justify-between items-center mt-4 mb-3 sm:mb-4'>

          <h1 className='text-xl font-bold md:text-2xl lg:text-3xl '>
            Daily Deals
        </h1>

          <p className='text-xs sm:text-sm flex items-center gap-1 cursor-pointer'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5'>
        {
            products.map((product, index) => {
                ...
                const handleAddHomeSection = async () => {
                    ...
                    try { 
                        await api.put(`/products/home-sections/delete/${product?.slug}?section=dailyOffer`); 
                        toast.success(<span className="capitalize">{`${product.name} delete to daily offer section successfully.`}</span>)
                        handleGetPopularSection();
                    } catch (error) {
                            ...
                            <div className='mt-4 flex justify-end'>
                                ...
                                <div className="flex justify-center items-center">
                                    <button onClick={handleAddHomeSection} className="font-semibold text-[12px] btn btn-sm btn-error flex transform transition-all hover:scale-105 duration-200 hover:cursor-pointer">
                                        ...
                                        {loadingId === product._id ? 'loading...' : 'Delete Offer Section'}
                                    </button>
                                </div>
